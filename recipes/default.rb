...
raise "Add a ['copperegg']['apikey'] attribute to configure this node's Collector." if node['copperegg'] && node['copperegg']['apikey'].nil?
...
node.set['copperegg']['alltags'] = node['copperegg']['tags'] + chef_server_tags

Chef::Log.warn('Setting UUID to FQDN:\n')
node.set['copperegg']['fqdn'] = node['fqdn'] if node['copperegg']['use_fqdn']
